...
// NewTicketStore construye el repositorio sobre la colección de tickets del
// core. La base y la colección se inyectan porque viven en otra DB del mismo
// cluster que la del engine (ver config.CoreDatabaseName / CoreTicketsCollection).
func NewTicketStore(db *mongo.Database, collection string) *TicketStore {
	return &TicketStore{
...
// juego, ordenados por _id ascendente. Ver ports.TicketRepository para la regla
// de por qué el filtro es PENDING y no "!= CANCELED".
func (r *TicketStore) FindPendingByGame(ctx context.Context, gameID string, cursor *string, limit int64) ([]domain.Ticket, *string, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("error finding pending tickets for game %s: %w", gameID, err)
	}
...
// UpdateTickets persiste win, state y las balotas sorteadas de los tickets
// recibidos en un solo BulkWrite. metadata.balls se setea con dot-notation para
// NO pisar metadata.bets. No toca version (para no interferir con el control de
// concurrencia optimista del core).
	...
	for _, ticket := range tickets {
		...
		update := bson.M{
			"$set": bson.M{
				"win":            ticket.Win,
				"state":          ticket.State,
				"metadata.balls": ticket.Metadata.Balls,
			},
		}
