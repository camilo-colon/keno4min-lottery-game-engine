...
import (
	...
	"github.com/cronos/keno4min-lottery-game-engine/functions/update-tickets-core/internal/secrets"
)
...
type Config struct {
	...
	DatabaseName string
	Stage        string
	// CoreDatabaseName y CoreTicketsCollection apuntan a la colección de tickets
	// del microservicio core, que vive en OTRA base del mismo cluster que la del
	// engine (reutiliza el mismo secret / cliente Mongo).
	CoreDatabaseName      string
	CoreTicketsCollection string
}
...
func Load(ctx context.Context) (*Config, error) {
	...
	cfg.CoreDatabaseName = getEnv("CORE_DATABASE_NAME", "core")
	cfg.CoreTicketsCollection = getEnv("CORE_TICKETS_COLLECTION", "tickets")
