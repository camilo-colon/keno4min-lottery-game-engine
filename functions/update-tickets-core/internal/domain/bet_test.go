...
func TestBetPayout(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			bet:   domain.Bet{Money: 50000, Nums: []int64{33, 54, 24}},
		...
		},
		{
			...
			bet:   domain.Bet{Money: 10000, Nums: []int64{7}},
			balls: mask(7, 9, 11),
		...
		},
		{
			...
			bet:   domain.Bet{Money: 333, Nums: []int64{7}},
			balls: mask(7),
			want:  1_165, // 333 * 350 / 100 = 1165.5 → 1165
		},
		{
			name:  "apuesta vacía no paga",
			bet:   domain.Bet{Money: 50000, Nums: []int64{}},
			balls: mask(7, 9),
			want:  0,
		},
	}
