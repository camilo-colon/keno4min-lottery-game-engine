...
func TestNewBitmask(t *testing.T) {
	tests := []struct {
		...
		{"vacío", nil, domain.Bitmask{}},
		{"números en mask1", []int64{1, 2, 3}, mask(1, 2, 3)},
		{"números altos en mask2", []int64{65, 80}, mask(65, 80)},
		{"a ambos lados del bit 64", []int64{64, 65}, mask(64, 65)},
		{"ignora fuera de rango 1..80", []int64{0, 7, 81, -3, 9}, mask(7, 9)},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := domain.NewBitmask(tt.nums); got != tt.want {
				t.Errorf("NewBitmask(%v) = %+v, want %+v", tt.nums, got, tt.want)
			}
		})
	}
}
